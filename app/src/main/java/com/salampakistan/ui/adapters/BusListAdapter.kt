...
import com.salampakistan.model.getbusservicesreponse.Facility
...
class BusListAdapter<VDB, M>(private val context: Context, @LayoutRes private val resId: Int) :
    ...
    private lateinit var adapterServices: SimpleListAdapter<RowServicesBinding, Facility>
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        RxView.clicks((holder.binding as RowBusBinding).root)
            .subscribe {
                selectedIndex = (holder.binding).model!!.id
                notifyDataSetChanged()
            }

        adapterServices = SimpleListAdapter(R.layout.row_services)
        val layoutManager = GridLayoutManager(context, 3)
        (holder.binding as RowBusBinding).recServices.layoutManager = layoutManager
        (holder.binding as RowBusBinding).recServices.adapter = adapterServices
        adapterServices.updateContent((holder.binding as RowBusBinding).model?.facilities)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if ((binding as RowBusBinding).model!!.id == selectedIndex) {
